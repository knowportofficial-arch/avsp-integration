...
import com.avsp.pro.capture.camera.analyzer.ExposureState
import com.avsp.pro.capture.camera.analyzer.FocusState
import com.avsp.pro.capture.camera.analyzer.ShotAnalysisResult
...
import com.avsp.pro.capture.camera.candidate.CandidateRanker
...
import com.avsp.pro.capture.camera.engine.CinematographerDecisionEngine
...
import com.avsp.pro.capture.camera.planner.ShotPlanner
...
import com.avsp.pro.capture.camera.mission.ShotMissionGenerator
import com.avsp.pro.capture.camera.mission.ShotStatus
import com.avsp.pro.capture.camera.mission.ShotMediaType
import com.avsp.pro.capture.camera.mission.UserReportedEvent
...
import com.avsp.pro.capture.camera.understanding.SceneUnderstandingEngine
import com.avsp.pro.capture.camera.understanding.SceneUnderstandingResult
import com.avsp.pro.capture.camera.understanding.SubjectUnderstandingEngine
import com.avsp.pro.capture.camera.understanding.SubjectUnderstandingResult
...
class CameraViewModel(
        ...
        if (id.isNotBlank() && projectRepository != null) {
            viewModelScope.launch {
                when (val result = projectRepository.getProjectById(id)) {
                    is com.avsp.pro.capture.common.Resource.Success -> {
    ...
    fun loadMasterShotPlan(plan: com.avsp.pro.capture.camera.planner.MasterShotPlan) {
        val mission = com.avsp.pro.capture.camera.planner.ShotMissionPlanAdapter.toShotMission(plan)
    ...
    fun keepPendingMedia(onCompleted: () -> Unit) {
        ...
        viewModelScope.launch {
            ...
            if (currentShot != null) {
                val candidatePlaceholder = com.avsp.pro.capture.camera.candidate.ShotCandidate(
                    id = currentShot.id,
